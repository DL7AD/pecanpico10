	...
	try:
		...
		db.cursor().execute(
			"""INSERT INTO `position` (`call`,`rxtime`,`org`,`adc_vsol`,`adc_vbat`,`pac_vsol`,`pac_vbat`,`pac_pbat`,`pac_psol`,`light_intensity`,`gps_lock`,
				...
				VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE `id`=`id`""",
		...
		# Debug
		Terminal().info('Received %s packet packet Call=%s Reset=%d ID=%d' % (typ, call, reset, _id))

		return {'type': 'pos', 'reset': reset, 'id': _id}

	except struct.error:

		Terminal().error('Received erroneous %s packet Call=%s' % (typ, call))
...
def insert_directs(db, call, dir, rxtime):
	...
	# Debug
	Terminal().info('Received dir packet packet Call=%s' % call)
