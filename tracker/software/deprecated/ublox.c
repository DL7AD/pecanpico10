...
bool gps_get_sv_info(gps_svinfo_t *svinfo, size_t size) {
  ...
  if(!gps_receive_payload(0x01, 0x30, (unsigned char*)svinfo, size,
                          TIME_MS2I(5000))) { // Receive request
...
uint8_t gps_disable_nmea_output(void) {
	uint8_t nonmea[] = {
		0xB5, 0x62, 0x06, 0x00, 60/*20*/, 0x00,	// UBX-CFG-PRT
		0x01, 0x00, 0x00, 0x00, 			// UART1, reserved, no TX ready
		0xD0, 0x08, 0x00, 0x00,				// UART mode (8N1)
		0x80, 0x25, 0x00, 0x00,				// UART baud rate (9600)
#if UBLOX_USE_I2C == FALSE
		0x01, 0x00,							// input protocols (uBx only)
		0x01, 0x00,							// output protocols (uBx only)
#else
        0x00, 0x00,                         // input protocols (none)
        0x00, 0x00,                         // output protocols (none)
#endif
		0x00, 0x00,							// flags
		0x00, 0x00,							// reserved
#if 1
        0x00, 0x00, 0x00, 0x00,             // DDC, reserved, no TX ready
        0x42, 0x00, 0x00, 0x00,             // Slave address
        ...
        0x00, 0x00,                         // reserved
#if 0
        0x04, 0x00, 0x00, 0x00,             // SPI, reserved, no TX ready
        ...
        0x00, 0x00,                         // reserved
#endif
        0x03, 0x00, 0x00, 0x00,             // USB, reserved, no TX ready
        0x00, 0x00, 0x00, 0x00,             // reserved
        0x00, 0x00, 0x00, 0x00,             // reserved
        0x00, 0x00,                         // input protocols (none)
        0x00, 0x00,                         // output protocols (none)
