...
int main(void) {
  ...
  while (true) {
    if(!palReadPad(PIOB, PIOB_USER_PB)) {
      rtcGetTime(&RTCD0, &now_DateTime);
      rtcConvertDateTimeToStructTm(&now_DateTime, &now_tm, NULL);
      strftime(buffer, SIZE, "%Y-%m-%d %H:%M:%S", &now_tm);
      chprintf((BaseSequentialStream *) &SD1,"Current time:%s\n\r",buffer);
    }
