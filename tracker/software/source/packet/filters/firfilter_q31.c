 ...
 * @brief   Creates a Q31 fixed point FIR filter.
 *
 * @param[in] filter        pointer to a @p QFIREmbeddedFilter structure
 * @param[in] instance      pointer to a @p arm_fir_instance_q31 structure
 * @param[in] numTaps       the number of taps in the filter
 * @param[in] pCoeffs       pointer to array of q31 filter coefficients
 * @param[in] pState        pointer to q31 state values used by filter
...
void create_qfir_embedded_filter(qfir_emb_filter_t *filter,
  ...
  reset_qfir_embedded_filter(instance);
}
...
void apply_qfir_embedded_filter(qfir_emb_filter_t *filter,
  ...
  /* For temporary copy of input data. */
  q31_t input_copy[PKT_QFIR_BLOCK_SIZE];

  /* Scale the input(s) down. */
  arm_scale_q31(input, Q31_MAX, -filter->scale,
                input_copy, PKT_QFIR_BLOCK_SIZE);

  /*
   * Apply the scaled input(s) to the filter and compute the output result(s).
   */
  arm_fir_q31(&filter->instance, input_copy,
              output, PKT_QFIR_BLOCK_SIZE);

  /* Scale the output(s) up. */
  arm_scale_q31(output, Q31_MAX, filter->scale, output, PKT_QFIR_BLOCK_SIZE);
}
