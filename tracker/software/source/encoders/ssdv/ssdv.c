...
static char ssdv_have_marker_data(ssdv_t *s)
{
	...
	case J_SOF0:
		...
		if(d[5] != 1 && d[5] != 3)
		{
			TRACE_ERROR("SSDV > The image must have 1 or 3 components");
			return(SSDV_ERROR);
		}
...
char ssdv_dec_feed(ssdv_t *s, uint8_t *packet)
{
	...
	if(packet_id != s->packet_id)
	{
		if(packet_id < s->packet_id)
		{
			...
			TRACE_ERROR("SSDV > Packets are not in order. %i > %i", s->packet_id - 1, packet_id);
			return(SSDV_FEED_ME);
		}
		
		/* One or more packets have been lost! */
		TRACE_WARN("Gap detected between packets %i and %i", s->packet_id - 1, packet_id);
	...
	for(; i < s->pkt_size_payload; i++)
	{
		if(i == s->packet_mcu_offset)
		{
			...
			{
				TRACE_ERROR("SSDV > Unexpected MCU ID in packet %d.", packet_id);
				return(SSDV_FEED_ME);
			}
		}
...
char ssdv_dec_is_packet(uint8_t *packet, int *errors)
{
	...
	if(pkt[1] == 0x66 + SSDV_TYPE_NOFEC)
	{
		...
		if(x == (uint32_t)(pkt[i + 3] | (pkt[i + 2] << 8) | (pkt[i + 1] << 16) | (pkt[i] << 24)))
		{
